...
static PyObject *enable(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "siOI:enable", &name, &startnum, &failnum, &failinfo,
				&flags))
		return NULL;

	/* See failinfo()'s comment regarding failinfo's RC */
	return PyLong_FromLong(fiu_enable(name,startnum, failnum, failinfo, flags));
}
...
static PyObject *enable_random(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "siOId:enable_random", &name,&startnum, &failnum,
				&failinfo, &flags, &probability))
		return NULL;

	/* See failinfo()'s comment regarding failinfo's RC */
	return PyLong_FromLong(fiu_enable_random(name,startnum, failnum, failinfo,
				flags, probability));
}


static int external_callback(const char *name, int *startnum, int *failnum, void **failinfo,
		unsigned int *flags)
{
	int rv;
	PyObject *cbrv;
	...
	args = Py_BuildValue("(siI)", name, *startnum, *failnum, *flags);
	if (args == NULL) {
...
static PyObject *enable_external(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "siIO:enable_external", &name, &startnum, &failnum,
	...
	return PyLong_FromLong(fiu_enable_external(name, startnum, failnum,
...
static PyObject *enable_stack_by_name(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "siOIs|i:enable_stack_by_name",
				&name, &startnum, &failnum, &failinfo, &flags,
				&func_name, &pos_in_stack))
		return NULL;

	return PyLong_FromLong(fiu_enable_stack_by_name(name, startnum, failnum,
				failinfo, flags,
				func_name, pos_in_stack));
}
