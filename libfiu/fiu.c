...
static struct pf_info *pf_create(const char *name, int startnum, int failnum,
...
int fiu_fail(const char *name)
{
	...
	switch (pf->method) {
		case PF_START_FROM:
			pf->run_times ++;
			if (pf->run_times > pf->startnum) {
				fprintf(stderr, "%s fail runtime %d, startnum %d\n", pf->name, pf->run_times, pf->startnum);
				goto exit_fail;
			} else {
				fprintf(stderr, "%s normal runtime %d, startnum %d\n", pf->name, pf->run_times, pf->startnum);
				goto exit;
			}
			break;
		case PF_ALWAYS:
...
int fiu_enable(const char *name, int startnum, int failnum, void *failinfo,
		unsigned int flags)
{
	struct pf_info *pf;
	if (startnum == 0) 
		pf = pf_create(name, startnum, failnum, failinfo, flags, PF_ALWAYS);
	else
		pf = pf_create(name, startnum, failnum, failinfo, flags, PF_START_FROM);
	if (pf == NULL)
		return -1;
...
int fiu_enable_random(const char *name, int startnum, int failnum, void *failinfo,
		unsigned int flags, float probability)
{
	struct pf_info *pf;

	pf = pf_create(name, startnum, failnum, failinfo, flags, PF_PROB);
	if (pf == NULL)
		return -1;
...
int fiu_enable_external(const char *name, int startnum, int failnum, void *failinfo,
		unsigned int flags, external_cb_t *external_cb)
{
	struct pf_info *pf;

	pf = pf_create(name, startnum, failnum, failinfo, flags, PF_EXTERNAL);
	if (pf == NULL)
		return -1;
...
int fiu_enable_stack(const char *name, int startnum, int failnum, void *failinfo,
		unsigned int flags, void *func, int func_pos_in_stack)
{
	...
	pf = pf_create(name, startnum, failnum, failinfo, flags, PF_STACK);
	if (pf == NULL)
		return -1;
...
int fiu_enable_stack_by_name(const char *name, int startnum, int failnum, void *failinfo,
		...
		int func_pos_in_stack)
{
	...
	return fiu_enable_stack(name, startnum, failnum, failinfo, flags, fp,
			func_pos_in_stack);
}
